...
def build_spectrogram_animation(filename, fft_size, x_range=None, y_range=None):
	...
	def draw_frame(frame, fig, graph, line, spec):
		...
		#Indice, quand générateur vide, raise exception: StopIteration
...
def main():
	...
	# xs = [x for x in range(10)] #Axe des X
	# ys = [x**2 for x in xs] # Axe des Y
	# plt.plot(xs, ys)
	# plt.show()
	# return
	...
	y_axis, x_axis = apply_fft(block_chord, SAMPLING_FREQ)
	...
	for note in notes:
		y_axis, x_axis = apply_fft(note, SAMPLING_FREQ)
		plt.plot(x_axis, y_axis)
	plt.show()
